  ...
  <div class="config-bool-switch">
    <div class="config-field-row">
      ...
      <div class="config-controls">
        <el-switch
          :model-value="config.current_value as boolean"
        ...
        <el-button
          ...
          @click="handleReset"
        >恢复默认</el-button>
        <span class="config-default">默认: {{ config.default_value ? '开' : '关' }}</span>
      </div>
    </div>
...
async function handleChange(val: boolean) {
  if (val === Boolean(props.config.default_value)) {
...
<style scoped>
.config-bool-switch { padding: 12px 0; border-bottom: 1px solid var(--el-border-color-light); }
.config-bool-switch:last-child { border-bottom: none; }
