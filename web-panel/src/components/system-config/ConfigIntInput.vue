<template>
  <div class="config-int-input">
    ...
    <div v-if="config.updated_at" class="config-meta">
      修改于 {{ formatTime(config.updated_at) }}
    </div>
  </div>
</template>
...
watch(
  () => props.config.current_value,
  (nv) => { localValue.value = nv as number },
)
...
<style scoped>
.config-int-input { padding: 12px 0; border-bottom: 1px solid var(--el-border-color-light); }
.config-int-input:last-child { border-bottom: none; }
